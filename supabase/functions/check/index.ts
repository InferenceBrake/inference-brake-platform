...
const model = new Supabase.ai.Session("gte-small");
	...
	try {
		...
		// Skip usage tracking for test mode
		let currentChecks = user.checks_today || 0;
		let dailyLimit = user.daily_limit || 1000;

		// Only increment and check rate limits for non-test mode
		if (!isTestMode) {
			await supabase
				.rpc("increment_usage", {
					user_uuid: user.id,
			...
			currentChecks = updatedUser?.checks_today || currentChecks + 1;
			dailyLimit = updatedUser?.daily_limit || dailyLimit;
